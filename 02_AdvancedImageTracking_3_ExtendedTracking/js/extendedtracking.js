/*
    ...
    The url argument is ignored. Dummy sensor values are returned instead. Implement request handling here to fetch data
    from custom origin
    The callback is called once new values are available. Note that the applications expect them to be in the format
    defined in SENSOR_AUGMENTATION_META
 */
var requestDataFromServer = function(url, callback, errorCallback) {
    ...
    /*
        Use e.g. XMLHttpRequest to parse your server response. Let's simulate a server response within 100 milli
        seconds.
     */
    setTimeout(function() {
...
var World = {
    ...
    init: function() {
        ...
        try {
            ...
            /*
                Show pipes model as augmentation. Note: This is usually replaced by a silhouette model or not used at
                all.
             */
            ...
            var tracker = new AR.ImageTracker(targetCollectionResource, {
                onTargetsLoaded: function() {
                    ...
                    try {
                        /*
                            add AR.ImageTrackable once tracker is active. Using wild-card as targetname to augment
                            every target in the wtc file the same way
                         */
                        World.imageTrackable = new AR.ImageTrackable(tracker, "*", {
                            enableExtendedTracking: true,
                            /* activates extended tracking */
                            drawables: {
                                cam: World.camDrawables /* initially no camDrawables exist, they are added and */
                                /* updated once sensor values arrive */
                            },

                            /* callback function indicating quality of SLAM tracking. */
                            onExtendedTrackingQualityChanged: function(targetName, oldQuality, newQuality) {
                            ...
                            /*
                                When losing the target -> snapToScreen is enabled and the close button appears,
                                so user can dismiss rendering of the video.
                             */
                        ...
                        World.updateSensorValuesFromRemote(
                            World.SENSOR_SERVICE_URL, World.REFRESH_RATE_MS, World.SENSOR_AUGMENTATION_META);
                    } catch (err) {
                        World.onError(err);
                    }
                },
        ...
        for (var i = 0; i < groupIdentifiers.length; i++) {
            ...
            if (META[groupId]) {
                ...
                if (!World.AR_LABELS[groupId].title) {
                    ...
                    var titleText = new Array(META[groupId].boxWidthChars).join(" ");
                    var titleBG = new AR.Label(titleText, META[groupId].textSize, {
                        translate: META[groupId].translate,
                        zOrder: 0,
                ...
                for (var v = 0; v < groupValueIds.length; v++) {
                    ...
                    if (!World.AR_LABELS[groupId].values[groupValueId]) {
                        ...
                        /* define background as label so that we do not need to load any background image */
                        var rowText = new Array(META[groupId].boxWidthChars).join(" ");
                        var rowBG = new AR.Label(rowText, META[groupId].textSize, {
                            translate: META[groupId].translate,
                            zOrder: 0,
    ...
    /*
        The serviceUrl is used to request sensor data from.
        IntervalMillis in milliseconds to wait between updating augmentations and requesting values from serviceurl
        again.
        The meta parameter defines how to interpret the sensor values and create augmentations accordingly
     */
    updateSensorValuesFromRemote: function(serviceUrl, intervalMillis, meta) {
        ...
        var onServerResponseArrivedFn = function(serverResponse) {
            ...
            /*
                trigger next update after predefined refresh rate. This way we ensure to first process server
                response and then triggering the next request
             */
            setTimeout(function() {
